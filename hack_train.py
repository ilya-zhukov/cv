...
def parse_arguments():
    ...
    parser.add_argument("--continue_calc", "-c", help="continue calculation", action="store_true")
    parser.add_argument("--learning-rate", "-lr", default=1e-3, type=float)
    parser.add_argument("--gpu", action="store_true")
    parser.add_argument("--not_final", "-nf", help="do final calculation", action="store_true")    
    parser.add_argument("--rounds", "-r", default=1, type=int)
...
def main(args):
    ...
    for ri in range(args.rounds):
        print(f"Reading data {time.strftime('%H:%M:%S')} ...")
        ...
        print(f"Creating model {time.strftime('%H:%M:%S')} ...")
        device = torch.device("cuda: 0") if args.gpu else torch.device("cpu")
    
        if args.continue_calc:
            with open(f"{args.name}_step.pth", "rb") as fp:
                model = torch.load(fp)
        else:
        ...
        # 2. train & validate
        print(f"Ready for training {time.strftime('%H:%M:%S')} ...")
        best_val_loss = np.inf
        for epoch in range(args.epochs):
            train_loss = train(model, train_dataloader, loss_fn, optimizer, device=device)
            val_loss = validate(model, val_dataloader, loss_fn, device=device)
            print("Epoch #{:2}:\ttrain loss: {:5.2f}\tval loss: {:5.3f}".format(epoch, train_loss, val_loss))
            if val_loss < best_val_loss:
                best_val_loss = val_loss
                with open(f"{args.name}_best.pth", "wb") as fp:
                    torch.save(model.state_dict(), fp)
                    
            with open(f"{args.name}_step.pth", "wb") as fp: # save step
                torch.save(model, fp)
                
            #val_predictions = predict(model, val_dataloader, device)    
            #with open(f"{args.name}_val_predictions.pkl", "wb") as fp:
            #    pickle.dump({"image_names": val_dataset.image_names,
            #                 "landmarks": val_predictions}, fp)   
                
        # 3. predict
        
    if not args.not_final:
